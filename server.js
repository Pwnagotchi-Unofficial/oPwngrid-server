...
//Create timed refresh data variables
...
app.use(log)
app.use(cors({ origin: 'https://' + process.env.DOMAIN }));
...
app.get('*', function (req, res, next) {
  if (req.headers.host == process.env.DOMAIN) { //if it's a sub-domain
  ...
  } else if (req.headers.host == 'api.' + process.env.DOMAIN) {
...
app.get('/api/statistics/builds', async (req, res) => {
  console.log("Got: stats/builds")
  //get data if updated at was 10 minutes or longer ago
  if (refreshBuilds == null || refreshBuilds > Date.now() + (30 * MS_PER_MINUTE)) {
    refreshBuilds = Date.now()
    try {
      [rows, fields] = await conn.execute('SELECT json_extract(data, \'$.build\') AS build FROM units WHERE (`updated_at` > DATE_SUB(now(), INTERVAL 10 DAY))');
    } catch (err) {
      console.log(err)
      res.status(500).json({ "error": "Internal Server Error" })
      return
