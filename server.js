...
app.get('/api/v1/unit/inbox/:messageId', authenticate, (req,res) => {
  console.log("Got: /api/v1/unit/inbox/:messageId")
...
app.post('/api/v1/unit/:fingerprint/inbox',toJson, authenticate, (req,res) => {
  if (res.locals.authorised) {
    console.log(res.locals)
      console.log(req.params.fingerprint,res.locals.author.unit_ident[0],res.locals.author.unit_ident[1],req.body.data,req.body.signature)
    ...
    function(err, results, fields) {
      ...
      if (results.length == 1) {
        ...
        connection.query('INSERT INTO aps (bssid, essid, identity) VALUES (UNHEX(REPLACE(?, \':\',\'\' )), ?,?)',
          [req.body.bssid, req.body.essid, res.locals.author.unit_ident],
          function(err, results, fields) {
            if (err) {
  ...
  req.body.forEach(function(ap){
    ...
    function(err, results, fields) {
      ...
      if (results.length == 1) {
        ...
        // Because no APs exist with that SSID, add it to the database.
        connection.query('INSERT INTO aps (bssid, essid, identity) VALUES (UNHEX(REPLACE(?, \':\',\'\' )), ?,?)',
          [ap.bssid, ap.essid, res.locals.author.unit_ident],
          function(err, results, fields) {
            if (err) {
              // Handle the error, but don't send a response here
              console.error(err);
